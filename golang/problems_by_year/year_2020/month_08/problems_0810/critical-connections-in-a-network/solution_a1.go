...
func criticalConnections_A1(n int, connections [][]int) [][]int {
	graph := make([][]int, n)
	...
	return dfs_a1(0, -1, 0, graph, make([]*int, n))
}

func dfs_a1(cur, prev, depth int, graph [][]int, visited []*int) (res [][]int) {
	...
	for _, n := range graph[cur] {
		...
		if visited[n] == nil {
			res = append(res, dfs_a1(n, cur, depth+1, graph, visited)...)
		}
