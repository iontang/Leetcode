...
//				System.out.println(cNum);
    ...
    //初始状态： d[0]=1,d[1]=0/1
    // 从第二位开始，i-1在1到9之间，i的值是dp[i] = main.java.dp[i]+ main.java.dp[i-1];
    // 对于两位数的：main.java.dp[i] = main.java.dp[i]+ main.java.dp[i-2];
...
//            System.out.println("second=" + second);
            if(first >= 1 && first <= 9) {
               dp[i] = dp[i]+ dp[i-1];  
               System.out.println("main.java.dp[i]first = " + dp[i]);
            }
            if(second >= 10 && second <= 26) {
                dp[i] = dp[i]+ dp[i-2];
                System.out.println("main.java.dp[i]second = " + dp[i]);
            }
        }
    	for(int j = 0;j<n+1;j++) {
